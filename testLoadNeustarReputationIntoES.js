...
//getDocCountOnIndex
app.get('/_cat/count/'+ scratchSpace, function (req, res) {
    winston.log("verbose", "Got a GET request for doc count on " + scratchSpace);

    uploadHttpSwitches['getDocCountOnIndex'] = true;
...
app.post('/' + neustarIpRegExp + '/' + metadataTypeName + '/_bulk', function (req, res) {
    ...
    if (resumeTest) {
        if (JSON.parse(reqBody[1]).start_ip_int == (((resumeFrom * (resumeFrom+1))/2) + resumeFrom + 1)) {
            winston.log("verbose", "Resume test start IP of first row is " + JSON.parse(reqBody[1]).start_ip_int);
        } else {
            throw new Error("Resume test start IP of first row is not " + (((resumeFrom * (resumeFrom+1))/2) + resumeFrom + 1));
        }
    } else if (uploadTest) {
        if (JSON.parse(reqBody[1]).start_ip_int == 1) {
        ...
        if ((JSON.parse(reqBody[(maxLimit*2) - 1]).start_ip_int) == (maxLimit * (maxLimit+1))/2) {
